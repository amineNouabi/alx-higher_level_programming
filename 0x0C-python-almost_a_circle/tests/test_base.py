...
class TestBase(unittest.TestCase):
    ...
    def test_from_json_string(self):
        ...
        self.assertEqual([{'id': 12}], Base.from_json_string('[{"id": 12}]'))
    ...
    def test_load_from_file(self):
        ...
        list_rectangles_input = [
            Rectangle(2, 3, 4, 5, 1),
            Rectangle(7, 8, 9, 10, 6),
            Rectangle(12, 13, 14, 15, 11),
            Rectangle(17, 18, 19, 20, 16),
            Rectangle(22, 23, 24, 25, 21),
            Rectangle(27, 28, 29, 30, 26)
        ]
        Rectangle.save_to_file(list_rectangles_input)
        list_rectangles_output = Rectangle.load_from_file()
        map(lambda obj: print(obj), list_rectangles_output)
        self.assertEqual(type(list_rectangles_output), list)
        self.assertEqual(len(list_rectangles_output), 6)
        self.assertEqual(type(list_rectangles_output[0]), Rectangle)
        self.assertEqual(list_rectangles_output[0].id, 1)
        self.assertEqual(list_rectangles_output[0].width, 2)
        self.assertEqual(list_rectangles_output[0].height, 3)
        self.assertEqual(list_rectangles_output[0].x, 4)
        self.assertEqual(list_rectangles_output[0].y, 5)
        self.assertEqual(list_rectangles_output[5].id, 26)
        self.assertEqual(list_rectangles_output[5].width, 27)
        self.assertEqual(list_rectangles_output[5].height, 28)
        self.assertEqual(list_rectangles_output[5].x, 29)
        self.assertEqual(list_rectangles_output[5].y, 30)
        list_rectangles_input = []
        Rectangle.save_to_file(list_rectangles_input)
        list_rectangles_output = Rectangle.load_from_file()
        self.assertEqual(type(list_rectangles_output), list)
        ...
        self.assertEqual(type(list_rectangles_output), list)
        self.assertEqual(len(list_rectangles_output), 0)
        list_rectangles_input = [Rectangle(2, 3, 4, 5, 1)]
        Rectangle.save_to_file(list_rectangles_input)
        list_rectangles_output = Rectangle.load_from_file()
        self.assertEqual(type(list_rectangles_output), list)
